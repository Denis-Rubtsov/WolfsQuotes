...
Configuration comes from `Vlk/appsettings.json` overridden by environment variables: `TelegramBot:Token`, `ADMIN_IDS` (comma/semicolon/space-separated), `AiApikey`, `QuotesFile`, `BASIC_URL` (voice audio base URL for inline mode), `SystemPromptFile` (optional; re-read on every generation, so prompt edits apply without restart), `StarsPrice` (Telegram Stars price of one over-limit generation, default 10, 0 disables selling). The README documents these and every bot command — update it when they change.
